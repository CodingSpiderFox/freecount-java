...
export const ProjectMember = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const projectMemberList = useAppSelector(state => state.projectMember.entities);
  const loading = useAppSelector(state => state.projectMember.loading);
  const totalItems = useAppSelector(state => state.projectMember.totalItems);
  const links = useAppSelector(state => state.projectMember.links);
  const entity = useAppSelector(state => state.projectMember.entity);
  ...
  return (
    <div>
      <h2 id="project-member-heading" data-cy="ProjectMemberHeading">
        <Translate contentKey="freecountApp.projectMember.home.title">Project Members</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="freecountApp.projectMember.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="freecountApp.projectMember.home.createLabel">Create new Project Member</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('freecountApp.projectMember.home.search')}
                />
          ...
          {projectMemberList && projectMemberList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th>
                    <Translate contentKey="freecountApp.projectMember.project">Project</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {projectMemberList.map((projectMember, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>
                      {projectMember.project ? <Link to={`project/${projectMember.project.id}`}>{projectMember.project.id}</Link> : ''}
                    </td>
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`${match.url}/${projectMember.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        ...
                        <Button
                          tag={Link}
                          to={`${match.url}/${projectMember.id}/edit`}
                          color="primary"
                          size="sm"
                        ...
                        <Button
                          tag={Link}
                          to={`${match.url}/${projectMember.id}/delete`}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="freecountApp.projectMember.home.notFound">No Project Members found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
